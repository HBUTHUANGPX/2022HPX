...
public:
    ros::NodeHandle n;
    ros::Publisher pub_odom = n.advertise<nav_msgs::Odometry>("odom", 10);
    serial::Serial ser;
    std::string data;
    ...
    ros::Subscriber sub_imu = n.subscribe<imu_serial::my_imu>("Imu_pub", 1, &OdomWorkFlow::doMsg, this);
    ros::Time _now, _old;
    ...
    void doMsg(const imu_serial::my_imu::ConstPtr &msg)
    {
        ...
        if (data.length() == 28 & flag == 0)
        {
            ...
            {
                byte_to_float(&ops_ori_pos_x, &ops_ori_pos_y, data);
        ...
        if (data.length() == 28 & flag == 1)
        {
            // if (data[0] == 0xD & data[1] == 0xA)
            // {
                byte_to_float(&abs_now_ops_pos_x, &abs_now_ops_pos_y, data);
                ...
                _old = _now;
                // cout<<(msg->Imu.header.stamp-ros::Time::now()).toSec()<<"   "<<(msg->Imu.header.stamp-ros::Time::now()).toNSec()<<endl;
            // }
        }
    ...
    void byte_to_float(float *px, float *py, std::string byte)
    {
        ...
        *px = pos.fdata[3] / 1000.0f;
        *py = pos.fdata[4] / 1000.0f;
    }
    typedef union
    {
        float fdata[6];
